...
class SolrManagementAPI:
    ...
    def add_field_types(self, field_types, bulk=True):
        schema_url = urljoin(
            self.base_url, f"/api/collections/{self.collection}/schema"
        )
        if bulk:
    ...
    def add_fields(self, fields, bulk=True):
        ...
        if bulk:
            ...
            for field in fields:
                ...
                if self.logging_verbose >= 2:
                    logger.info(
                        f"Creating field {field['name']} with type {field['type']}"
                    )
                self._make_post(schema_url, request)
    ...
    def create_copyfields(self, copyfields, bulk=True):
        ...
        if bulk:
            ...
            for copyfield in copyfields:
                ...
                if self.logging_verbose >= 2:
                    logger.info(
                        f"Creating copyfield {copyfield['source']}-{copyfield['dest']}"
                    )
                self._make_post(schema_url, request)
    ...
    def add_dynamic_fields(self, dynamic_fields, bulk=True):
        ...
        if bulk:
            ...
            for dynamic_field in dynamic_fields:
                ...
                if self.logging_verbose >= 2:
                    logger.info(
                        f"Creating dynamic field {dynamic_field['name']} with type {dynamic_field['type']}"
                    )
                self._make_post(schema_url, request)
    ...
    def delete_field_types(self, field_types, bulk=True):
        ...
        if bulk:
            request = {
                "delete-field-type": [
                    {"name": field_type} for field_type in field_types
                ]
            }
            if self.logging_verbose >= 2:
    ...
    def create_collection_and_schema(
        self, delete_default_fields_definition, schema_definition, test_field_name
    ):
        ...
        if not created:
            ...
            if has_field:
                if self.logging_verbose >= 1:
                    logger.info(
                        f"Collection {self.collection} already exists, not creating"
                    )
                return
            else:
                if self.logging_verbose >= 1:
    ...
    def disable_auto_data_driven_schema(self):
        ...
        if self.logging_verbose >= 1:
            logger.info(
                f"Disabled auto data driven schema for collection {self.collection}"
            )
    ...
    def create_collection_alias(self, alias):
        ...
        if self.logging_verbose >= 1:
            logger.info(
                f"Created collection alias {alias} for collection {self.collection}"
            )
